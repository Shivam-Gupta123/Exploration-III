...
void check_heading(float heading)
{
    if(RPS.Heading()>=0)
    {
        if(heading != 0)
        {
            ...
            {
                if((heading-3)>RPS.Heading())
                {
                    pulse_counterclockwise(PULSE_POWER,PULSE_TIME);
                }
                else if((heading+3)<RPS.Heading())
                {
        ...
        if(heading == 0)
        {
            while(RPS.Heading()<358 && RPS.Heading()>2)
            {
                if(RPS.Heading()<358 && RPS.Heading()>180)
                {
                    pulse_counterclockwise(PULSE_POWER,PULSE_TIME);
                }
...
int main(void)
{
    ...
    int A_B_counts, B_C_counts, C_D_counts, turn_90_counts, turn_180_counts;
    ...
    A_B_counts = COUNTS_PER_INCH*(B_y-A_y);
    B_C_counts = COUNTS_PER_INCH*(B_x-C_x);
    C_D_counts = COUNTS_PER_INCH*(D_x-C_x);
